...
class DataSet(object):
    ...
    def val_pickup(self):
        self.idx_val = np.random.randint(0, self.amount_val)

        data = np.expand_dims(np.load(self.list_validation_lr[self.idx_val]), axis=0)
        label = np.expand_dims(np.load(self.list_validation_hr[self.idx_val]), axis=0)
