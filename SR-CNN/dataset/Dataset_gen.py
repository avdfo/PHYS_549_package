...
cube_tot = 26
cube_training_num = 20
cube_validation_num = 5
cube_testing_num = 1
slice_num = 8
...
# Step 3. Set up the ARPES dictionary
    ...
    arpes = {'cube':{'X':[-k_bound,k_bound,300],'Y':[-k_bound,k_bound,300],'E':[-4.5,0.0,300],'kz':0.0}, 
        'hv':100,                          
        'T':10,                     
        'pol':np.array([1,0,-1]),           
        'SE':['constant',SE],            
...
# Step 4. Feed the basis, hamiltonian, and ARPES dictionaries to the Simulation class
    ...
    elif (cube_training_num) <= i < (cube_training_num + cube_validation_num): 
        sim.npy_from_cube(I, i, slice_num, -k_bound, 0., 'TB_validation', path=PATH)
        sim.npy_from_cube(Ig, i, slice_num, -k_bound, 0., 'ARPES_validation', path=PATH, noise=True, NSR=NSR)
        np.savetxt('%s/Parameters/Param_list_%d.txt' %(PATH, i), Param_list)
        for j in range(slice_num):
            a = np.load('%s/validation_hr/TB_sim_%d_%d.npy' %(PATH, i, j))
            b = np.load('%s/validation_lr/ARPES_sim_%d_%d.npy' %(PATH, i, j))
    ...
    elif (cube_training_num + cube_validation_num) <= i < (cube_tot): 
        sim.npy_from_cube(I, i, slice_num, -k_bound, 0., 'TB_test', path=PATH)
        sim.npy_from_cube(Ig, i, slice_num, -k_bound, 0., 'ARPES_test', path=PATH, noise=True, NSR=NSR)
        np.savetxt('%s/Parameters/Param_list_%d.txt' %(PATH, i), Param_list)
        for j in range(slice_num):
            a = np.load('%s/test_hr/TB_sim_%d_%d.npy' %(PATH, i, j))
            b = np.load('%s/test_lr/ARPES_sim_%d_%d.npy' %(PATH, i, j))
