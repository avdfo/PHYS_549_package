...
ground truth, TB data. Anti-aliasing process is also added to the TB data. To accelerate the data simulation process, we
also add a function generating ARPES data without considering matrix element effect.

The simulated data is directly saved to the dataset directory of the convolutional neural netwrok.
...
class simulation:
    ...
    def extract2D(self, datacube, slice_select, cal_type, NSR, noise=False, interp=False):
        ...
        a = np.squeeze(datacube[limits[1, 0]:limits[1, 1], limits[0, 0]:limits[0, 1], limits[2, 0]:limits[2, 1]])

        if cal_type == 'TB' or cal_type =='TB_train' or cal_type =='TB_validation' or cal_type =='TB_test':
            if interp:
                ...
                f = interp2d(xx, yy, a, kind='linear')
                a_new = f(x, y)
        ...
        elif cal_type == 'ARPES' or cal_type == 'ARPES_train' or cal_type == 'ARPES_validation' or cal_type == 'ARPES_test' or cal_type == 'ARPES_noise_free' or cal_type == 'ARPES_matrix':
            wg = (self.AE.cube[2][2] * self.AE.dE / (self.AE.cube[2][1] - self.AE.cube[2][0]) if abs(
                self.AE.cube[2][1] - self.AE.cube[2][0]) > 0 else 0)
            ...
            if noise:
                ...
                # noisemap = np.ones((self.cube[1][2],self.cube[0][2],self.cube[2][2]))
                # Ip = I+np.random.poisson(noisemap)*Imax
                Ip = np.random.poisson(a / Imax) / Imax
                # Ip = Ip/np.max(Ip)
                ...
                Ig = nd.gaussian_filter(a, (kg, wg))  # Only an instrumetnal resolution limit filter
                Igmax = np.max(Ig)
                Ig = Ig / Igmax
                Ig = Ig * (np.sign(Ig) + 1.) / 2  # No Poison and circuit Gausian noise if noise=False
            return Ig
    ...
    def npy_from_cube(self, datacube, cube_idx, N, y_start, y_end, cal_type, path, noise=False, interp=True, NSR=0.8):
        ...
        elif cal_type == 'TB_train':
            makedir(path + '/train_hr')
            for i in range(N):
                TB_arr = self.extract2D(datacube, slice_select=('ky', y_start + i * np.abs(y_end-y_start)/(N-1)), cal_type=cal_type, NSR=NSR, noise=False, interp=interp)
                np.save("%s/train_hr/TB_sim_%d_%d" %(path, cube_idx, i), TB_arr)

        elif cal_type == 'ARPES_train':
            makedir(path + '/train_lr')
            for i in range(N):
                ARPES_arr = self.extract2D(datacube, slice_select=('ky', y_start + i * np.abs(y_end-y_start)/(N-1)), cal_type=cal_type, NSR=NSR, noise=noise)
                np.save("%s/train_lr/ARPES_sim_%d_%d" %(path, cube_idx, i), ARPES_arr)

        elif cal_type == 'TB_validation':
            makedir(path + '/validation_hr')
            for i in range(N):
                TB_arr = self.extract2D(datacube, slice_select=('ky', y_start + i * np.abs(y_end-y_start)/(N-1)), cal_type=cal_type, NSR=NSR, noise=False, interp=interp)
                np.save("%s/validation_hr/TB_sim_%d_%d" %(path, cube_idx, i), TB_arr)

        elif cal_type == 'ARPES_validation':
            makedir(path + '/validation_lr')
            for i in range(N):
                ARPES_arr = self.extract2D(datacube, slice_select=('ky', y_start + i * np.abs(y_end-y_start)/(N-1)), cal_type=cal_type, NSR=NSR, noise=noise)
                np.save("%s/validation_lr/ARPES_sim_%d_%d" %(path, cube_idx, i), ARPES_arr)

        elif cal_type == 'TB_test':
            makedir(path + '/test_hr')
            for i in range(N):
                TB_arr = self.extract2D(datacube, slice_select=('ky', y_start + i * np.abs(y_end-y_start)/(N-1)), cal_type=cal_type, NSR=NSR, noise=False, interp=interp)
                np.save("%s/test_hr/TB_sim_%d_%d" %(path, cube_idx, i), TB_arr)

        elif cal_type == 'ARPES_test':
            makedir(path + '/test_lr')
            for i in range(N):
                ARPES_arr = self.extract2D(datacube, slice_select=('ky', y_start + i * np.abs(y_end-y_start)/(N-1)), cal_type=cal_type, NSR=NSR, noise=noise)
                np.save("%s/test_lr/ARPES_sim_%d_%d" %(path, cube_idx, i), ARPES_arr)
